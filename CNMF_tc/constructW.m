...
if isfield(options,'Metric')%如果 field 是结构体数组 S 的一个字段的名称，则 TF = isfield(S,field) 返回 1。否则，将返回 0。
...
if ~isfield(options,'NeighborMode')
    options.NeighborMode = 'KNN';%默认为KNN
end

switch lower(options.NeighborMode)%lower将大写的字符串转化为小写，判断邻居模型是什么类型,KNN还是Supervised？
...
if ~isfield(options,'WeightMode')
    options.WeightMode = 'HeatKernel';%默认是heatkernel
end

bBinary = 0;
bCosine = 0;
switch lower(options.WeightMode)%判断是binary \ heatkernel \ cosine
    case {lower('Binary')}
        bBinary = 1; 
